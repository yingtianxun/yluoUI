var yluo_ScrollView = (function() {
	 ...
	 * showVertical  是否显示垂直滚动条,默认是true
	 * */
	function ScrollView(param) {
		...
		if(this.param.obj){
			this.oScrollView = this.param.obj;
		} else {
	...
	ScrollView.prototype._initScrollBar = function() {
		...
		// 初始化垂直滚动条的
		 this.param.showVertical && this._initVerticalLayout();
		// 初始化水平滚动条
		 this.param.showHorizontal && this._initHorizontalLayout();
		// 这里重新检查的了
		if(this.param.showHorizontal && this.param.showVertical){
			this._checkInitLayout();
		}
		// 设置滚动条高度宽度的
		this.param.showVertical && this._setLeftDropHeight();
		this.param.showHorizontal && this._setBottomDropWidth();
	};
	...
	ScrollView.prototype._setLeftDropHeight = function() {
		if (this.oLeftBar) {
			...
			leftDropHeight = leftDropHeight > 30 ? leftDropHeight : 30; // 设置滚动bar最小高度
